...
//#########################################################
async function mint(contractHash, metadata, publicKey_data) {
  contract.setContractHash(contractHash)
  var custom_key = getKeys()
  ...
  custom_key.publicKey.data = publicKey_data

  const args = RuntimeArgs.fromMap({

    "token_owner": custom_key.publicKey,
...
// Configuring body parser middleware1
...
app.post('/mintnft', (req, res) => {
    ...
    const book = req.body;
    const { address, nft_type, nft_count } = req.body;


    // Output the book to the console for debugging
    console.log(address);
    // books.push(book);
    var proper_user_address = "01" + address
	 const metadata = {
    "user_data": {
    	"address": proper_user_address
    },
    "nft_data": { "type": nft_type,
        "uri": "https://stochastic.life/public/img/instaprofile.jpg",
        "count": nft_count
    }
  }

  address_to_send_nft = hexToBytes(address)
	mint("hash-b9ca1f56c0729b4eac293485323009f5218634e9ff36da66a62dbdca935543c4", metadata, address_to_send_nft)

    res.send('NFT Added. It will show up in a while.');
});
